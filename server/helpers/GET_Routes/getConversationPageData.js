const getConversationPageData = (req, res, knex, user_id) => {
  ...
  const getConversationMessages = conversation => new Promise((resolve, reject) => {
    ...
    .select(
      'conversation_messages.id', 'conversation_messages.created_at as sent_at', 'conversation_messages.content',
      'conversation_messages.sender_id', 'users.photo_name', 'users.username as sender_name'
    )
